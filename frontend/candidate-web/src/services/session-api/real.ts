...
export const realSessionApiAdapter: CandidateSessionApiPort = {
  async resolveInvite(token: string): Promise<{ invite: Invite; drive: Drive; session: Session | null }> {
    ...
    const drive: Drive = {
      ...
      scheduleStart: payload?.scheduleStart || FIXTURE_DRIVE.scheduleStart,
      scheduleEnd: payload?.scheduleEnd || FIXTURE_DRIVE.scheduleEnd,
    };
  ...
  async submitFinalAssessment(sessionId: string): Promise<{ referenceId: string }> {
    try {
      ...
      const referenceId = res.data?.referenceId || `REF-${sessionId.slice(-6).toUpperCase()}-${Date.now().toString(36).toUpperCase()}`;
      return { referenceId };
    } catch (err: any) {
