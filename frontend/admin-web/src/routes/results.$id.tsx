            ...
            {(() => {
              ...
              return (
                <div className="space-y-4 pt-4 border-t border-line">
                  <h4 className="text-sm font-semibold text-ink">Recorded Incident Resolution &amp; Action Submissions</h4>
                  {simResponses.length === 0 ? (
                    <div className="p-4 bg-white border border-line rounded text-xs text-ink-tertiary italic">
                      No standalone simulation question responses recorded.
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {simResponses.map((resp: any, idx: number) => {
                        ...
                        const codePatch = payload.sourceCode || payload.code || payload.fixCode || payload.patch;
                        const summaryText = payload.summary || payload.sayText || payload.ticketReply || payload.initialSayText || payload.text;
                        const hasRunTests = payload.passedTests !== undefined || payload.totalTests !== undefined;
                        const passedTests = payload.passedTests || 0;
                        const totalTests = payload.totalTests || (payload.testCases ? payload.testCases.length : 1);
                        const isSuccess = payload.status === "SUCCESS" || payload.status === "COMPLETED" || (hasRunTests && passedTests === totalTests && totalTests > 0);
                        const statusStr = payload.status || (isSuccess ? "RESOLVED" : "SUBMITTED");
