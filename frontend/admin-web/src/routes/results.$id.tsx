...
function IndividualResultPage() {
  ...
  const availableTabs = useMemo(() => {
    ...
    const flagCount = (detail.integrityFlags || []).filter((f: any) => f.category !== "NO_ID_PROOF_ON_FILE").length;
  ...
  const flags = (detail.integrityFlags || []).filter((f: any) => f.category !== "NO_ID_PROOF_ON_FILE");
              ...
              {scenarioResponses.length === 0 ? (
                ...
                <div className="space-y-4">
                  {scenarioResponses.map((res: any, index: number) => {
                    ...
                    const evaluation = res.evaluation || res.responsePayload?.evaluation;
                    const scoreVal = typeof res.score === "number" ? Math.round(res.score <= 1.0 ? res.score * 100 : res.score) : typeof evaluation?.score === "number" ? Math.round(evaluation.score <= 1.0 ? evaluation.score * 100 : evaluation.score) : null;
