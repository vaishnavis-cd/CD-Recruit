      ...
      actions={
        <div className="flex items-center gap-2.5">
          <select
            ...
            className="px-3 py-2 text-[13px] font-medium text-ink focus:outline-none cursor-pointer border border-line rounded-lg bg-white hover:border-line-strong shadow-xs"
          >
      ...
      <div className="max-w-[1400px] mx-auto pb-12 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 bg-white border border-line rounded-xl p-6 shadow-sm flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-[14px] font-bold uppercase tracking-wider text-ink-2 flex items-center gap-2">
                  <Activity size={15} className="text-brand" /> Live Session Stream
                </h3>
                <p className="text-[12px] text-stext-2 mt-0.5">Real-time candidate assessment activities</p>
              </div>
            </div>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto max-h-[220px] pr-1">
              {filteredSessions.slice(0, 5).map((s: any) => {
                ...
                return (
                  <div key={s.id} className="flex items-center justify-between p-2.5 rounded-lg border border-bg-inset bg-bg-soft">
                    <div className="flex items-center gap-3">
                      <span className={`w-2.5 h-2.5 rounded-full ${isFlagged ? 'bg-danger' : s.status === 'submitted' ? 'bg-brand' : 'bg-emerald-500'}`} />
                      <div>
                        <div className="text-[13px] font-semibold text-ink">{s.candidate?.name || s.candidateName || 'Candidate'}</div>
                        <div className="text-[11px] text-stext-2">{s.roleTemplate?.roleName || 'Software Engineer'}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="font-mono text-[12px] font-bold text-ink">{s.compositeScore !== null ? `${s.compositeScore}%` : 'In progress'}</span>
                      <div className="text-[10px] text-stext-2">{s.submittedAt ? new Date(s.submittedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Active now'}</div>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="bg-white border border-line rounded-xl p-6 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h3 className="text-[16px] font-bold text-ink">Candidate Evaluation Roster</h3>
              <p className="text-[12px] text-stext-2">Actionable list of all assessment sessions requiring evaluation</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative">
                <input
                  ...
                  className="pl-8 pr-3 py-1.5 text-[12px] border border-line rounded-lg bg-bg-soft focus:outline-none focus:border-brand w-48"
                />
                <Search size={14} className="absolute left-2.5 top-2.5 text-stext-2" />
              </div>

              <div className="flex border border-line rounded-lg p-0.5 bg-bg-soft text-[12px]">
                {[
                  { id: "all", label: "All" },
                  ...
                  <button
                    ...
                    className={`px-3 py-1 rounded-md font-medium transition-colors cursor-pointer ${
                      rosterStatus === t.id ? "bg-white text-brand shadow-xs" : "text-ink-2 hover:text-ink"
                    }`}
                  >
                    {t.label}
                  </button>
          ...
          <div className="overflow-x-auto border border-line rounded-lg">
            <table className="w-full text-left text-[13px]">
              <thead className="bg-bg-soft text-ink-2 font-semibold text-[11px] uppercase tracking-wider border-b border-line">
                <tr>
              ...
              <tbody className="divide-y divide-line">
                {rosterSessions.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="py-8 text-center text-stext-2 text-[13px]">
                    ...
                    return (
                      <tr key={s.id} className="hover:bg-bg-soft transition-colors">
                        <td className="py-3 px-4 font-medium text-ink">
                          ...
                          <div className="text-[11px] text-stext-2">{s.candidate?.email || s.candidateEmail || "No email"}</div>
                        </td>
                        <td className="py-3 px-4 text-ink-2">
                          <div>{s.roleTemplate?.roleName || "Software Engineer"}</div>
                          <div className="text-[11px] text-stext-2">{s.driveName || "Drive Session"}</div>
                        </td>
                        ...
                        <td className="py-3 px-4 text-right">
                          <Link
                            to="/results/$id"
                            params={{ id: s.id }}
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-brand hover:bg-brand/90 text-white text-[12px] font-semibold rounded-lg transition-colors"
                          >
...
function SectionTitle({ children, noMargin, action }: { children: React.ReactNode; noMargin?: boolean; action?: React.ReactNode }) {
  return (
    <div className={`flex items-center justify-between ${noMargin ? "" : "mb-5"}`}>
      <h3 className="text-[11px] font-bold uppercase tracking-[0.15em] text-ink-2">
        {children}
      </h3>
...
function ReadoutTile({ label, value, suffix, tone }: { label: string; value: number; suffix?: string; tone: "ink" | "brand" | "amber" }) {
  ...
  return (
    <div className="border border-line bg-white rounded-xl p-5 flex flex-col justify-center shadow-sm">
      <div className="text-[11px] font-bold uppercase tracking-wider text-ink-2 mb-2">
        {label}
      </div>
      <div className="flex items-baseline gap-2">
        ...
        {suffix && <span className="text-[12px] font-medium text-stext-2">{suffix}</span>}
      </div>
    </div>
  );
}
...
function ActionCard({ icon, title, tone, description, children }: { icon: React.ReactNode, title: string, tone: 'danger' | 'warning' | 'info', description: string, children: React.ReactNode }) {
  ...
  return (
    <div className="bg-white border border-line rounded-xl p-5 flex flex-col shadow-sm">
      ...
      <p className="text-[12px] text-ink-2 mb-4">{description}</p>
...
function QueueItem({ title, subtitle, link, linkText, params }: any) {
  return (
    <div className="group flex items-center justify-between p-2 hover:bg-bg-soft rounded-lg transition-colors border border-transparent hover:border-line">
      <div>
        <div className="text-[13px] font-semibold text-ink">{title}</div>
        <div className="text-[11px] text-stext-2">{subtitle}</div>
      </div>
      <Link to={link} params={params} className="text-brand text-[12px] font-medium opacity-0 group-hover:opacity-100 flex items-center gap-1 transition-opacity">
...
function EmptyState({ text }: { text: string }) {
  return <div className="text-[12px] text-stext-2 italic py-3 text-center bg-bg-soft rounded-lg">{text}</div>;
}
        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={d.stage} className="flex items-center gap-4">
              <div className="w-28 text-[13px] font-medium text-ink-2 truncate" title={d.stage}>{d.stage}</div>
              <div className="flex-1 h-8 bg-bg-soft rounded-md overflow-hidden relative border border-bg-inset">
                <div
                  className="h-full bg-brand/20 rounded-r-md transition-all duration-500"
                  style={{ width: `${pct}%` }}
                >
                  <div className="h-full w-full border-r-[3px] border-brand" />
                </div>
                <span className="absolute inset-y-0 left-3 flex items-center text-[12px] font-mono font-semibold text-ink">
                  {d.count}
                </span>
              </div>
              <div className="w-16 flex justify-end">
                {i > 0 ? (
                  <span className="text-[10px] font-mono font-medium px-1.5 py-0.5 rounded bg-rose-50 text-danger">
                    −{drop}%
                  </span>
                ) : (
                  <span className="text-[10px] font-mono font-medium px-1.5 py-0.5 text-stext-2">
                    —
                  </span>
                )}
              </div>
...
function ScoreDistView({ sessions, roleFilter, setRoleFilter }: any) {
  ...
  return (
    <div className="h-full flex flex-col">
      <SectionTitle
        ...
        action={
          <select
            ...
            className="text-[11px] font-medium border border-line rounded-md px-2 py-1 bg-bg-soft text-ink-2"
          >
      ...
      <div className="flex-1 flex items-end gap-2 mt-6 border-b border-bg-inset pb-2">
        {dist.map((d) => (
          <div key={d.bucket} className="flex-1 flex flex-col items-center gap-2 group">
            <div className="font-mono text-[11px] text-stext-2 opacity-0 group-hover:opacity-100 transition-opacity">
              {d.count}
            </div>
            ...
            <div className="text-[11px] font-mono text-ink-2 mt-1">{d.bucket}</div>
          </div>
        ))}
      </div>
    </div>
...
function SayDoView({ sessions }: { sessions: any[] }) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <p className="text-[12px] text-ink-2 mb-4 leading-relaxed bg-bg-soft p-3 rounded-lg border border-bg-inset">
        <Sparkles size={14} className="inline mr-1.5 text-amber mb-0.5" />
      ...
      <div className="mt-auto grid grid-cols-5 gap-3">
        {dist.map((d) => (
          <div key={d.bucket} className="flex flex-col items-center text-center">
            <div className="text-[10px] font-mono uppercase text-stext-2 mb-1">{d.bucket}</div>
            <div className="font-mono text-[20px] font-semibold text-ink mb-2">{d.count}</div>
...
function TimeView({ data }: { data: any[] }) {
  ...
  return (
    <div className="h-full flex flex-col">
          ...
          <div key={d.module}>
            <div className="flex justify-between items-end text-[12px] mb-1.5">
              ...
              <span className="font-mono text-ink-2 text-[11px]">
                {fmt(d.avgSeconds)} <span className="text-stext-2 ml-1">(avg {fmt(d.cohortAvgSeconds)})</span>
              </span>
            </div>
            <div className="relative h-3 bg-bg-soft rounded-full overflow-hidden border border-bg-inset">
              {/* Baseline Track */}
              <div
                className="absolute inset-y-0 left-0 bg-brand/20 border-r border-brand/20"
      ...
      <div className="mt-5 flex gap-4 text-[11px] font-mono text-ink-2 bg-bg-soft w-fit px-3 py-1.5 rounded-md">
        <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-brand" /> Current</span>
        <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-brand/20" /> Baseline</span>
      </div>
    </div>
  );
}
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="overflow-x-auto mt-2">
        <div className="inline-grid gap-1.5 min-w-[500px]" style={{ gridTemplateColumns: `180px repeat(${severities.length}, 1fr)` }}>
          ...
          {severities.map((s) => (
            <div key={s} className="text-[10px] font-bold uppercase tracking-wider text-stext-2 text-center pb-2">
              {s}
            </div>
          ))}
          {categories.map((c) => (
            <Fragment key={c}>
              <div className="text-[12px] font-medium text-ink flex items-center">{c}</div>
              {severities.map((s) => {
                ...
                return (
                  <div
                    key={c + s}
                    className={`h-12 rounded-lg flex items-center justify-center font-mono text-[13px] font-medium border ${isCritical ? 'border-danger/20' : 'border-brand/10'} transition-transform hover:scale-[1.02] cursor-default`}
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 flex flex-col md:flex-row gap-8 items-center mt-4">
        ...
        <div className="flex-1 w-full flex flex-col justify-center">
          <div className="text-[10px] font-bold uppercase tracking-wider text-ink-2 mb-4">
            ...
            {data.overrides.map((o: any) => (
              <div key={o.direction}>
                <div className="flex justify-between text-[12px] mb-1.5">
                  ...
                  <span className="font-mono text-ink-2 font-medium">{o.count}</span>
                </div>
                <div className="h-2.5 bg-bg-soft rounded-full overflow-hidden border border-bg-inset">
                  <div
    ...
    <div className="h-full flex flex-col opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
      ...
      <div className="flex-1 flex flex-col justify-center relative mt-2">
        ...
        <div className="absolute inset-0 flex items-center justify-center backdrop-blur-[1px]">
          <div className="bg-white/90 px-4 py-2 rounded-lg border border-line text-[12px] font-mono text-ink-2 shadow-sm text-center">
