...
import { CompositeScoreResult, DriveModuleConfigEntry } from "@cd-recruit/shared-types";

export { CompositeScoreResult };
  ...
  async computeSessionScores(sessionId: string): Promise<CompositeScoreResult> {
    ...
    for (const resp of responses) {
      ...
      if (mod === "MCQ") {
        ...
        accuracy = (execResult?.passed || execResult?.matched || execResult?.status === "SUCCESS") ? 1.0 : 0.0;
    ...
    for (const resp of responses) {
      ...
      if (typeof sayValue === "number") {
        ...
        if (q.moduleType === "MCQ") {
          ...
          const execResult = payload?.executionResult;
          doValue = (execResult?.passed || execResult?.matched || execResult?.status === "SUCCESS") ? 1.0 : 0.0;
        } else {
          doValue = 0.85;
        }
